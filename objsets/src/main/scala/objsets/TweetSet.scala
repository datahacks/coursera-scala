...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(p => google.exists { x => p.text.contains(x) })
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(p => apple.exists { x => p.text.contains(x) })
