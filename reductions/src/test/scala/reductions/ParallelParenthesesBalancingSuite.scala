...
class ParallelParenthesesBalancingSuite extends FunSuite {
  ...
  test("balance should work for string of multiple length") {
    def check(input: String, expected: Boolean) =
      assert(balance(input.toArray) == expected,
        s"balance($input) should be $expected")

    check("(my name is lakhan(anil kapoor))", true)
    check(")earth is blue(nila)(", false)
    check("((open))", true)
    check("))close((", false)
    check(".)buster's mal heart))))))", false)
    check(".((((((((((((((((((((((", false)
    check("(.......))))))))))", false)
    check(").()()()()()()()()", false)
    check("()()()()()()()()", true)
  ...
  test("parBalance should work for string of multiple length") {
    def check(input: String, expected: Boolean) =
      assert(parBalance(input.toArray, 10) == expected,
        s"parBalance($input) should be $expected")
