...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =  {
    val ocset = occurrences map (x => (for (i <- 1 until (x._2+1)) yield (x._1, i)).toList)
    ocset.foldRight(List[Occurrences](Nil))((m,n) => n ++ (for(i <- m; j <- n) yield (i :: j)))
  }
  ...
  def sentenceAnagramsOcc(occ: Occurrences): List[Sentence] = {
    if (occ.isEmpty) List(Nil)
      ...
      for {
        ...
        k <- sentenceAnagramsOcc(subtract(occ, i))
      } yield j :: k
    }
  }
